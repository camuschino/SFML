...
var (
	targetPositionX, targetPositionY int
)

// ValidateMap function which works fine
func ValidateMap(algorithm string, player, target models.MapPoint, laberth models.Labyrinth, win *pixelgl.Window) (result bool) {
	fmt.Print("Algorithm: " + algorithm)
	imd := imdraw.New(nil)
...
// func checkMapByBFS(player, target models.MapPoint, imd *imdraw.IMDraw, win *pixelgl.Window) (validMap bool) {
// 	fmt.Print(player)
// 	fmt.Print(target)
// 	return
// }
...
func checkMapByBFS(player, target models.MapPoint, laberth models.Labyrinth, imd *imdraw.IMDraw, win *pixelgl.Window) (validMap bool) {
	queue := list.New()
	objectivePoint := models.MapPoint{XPoint: targetPositionX, YPoint: targetPositionY}
	...
	xCurrent := player.XPoint
	yCurrent := player.YPoint

	for currentPoint := queue.Front(); currentPoint == nil; {
